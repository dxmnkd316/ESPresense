...
float BMP280m5::readTemperature() {
    ...
    int32_t adc_T = read24(BMP280m5_REGISTER_TEMPDATA);
    adc_T >>= 4;

    var1 = ((((adc_T >> 3) - ((int32_t)_bmp280m5_calib.dig_T1 << 1))) *
            ((int32_t)_bmp280m5_calib.dig_T2)) >>
           11;

    var2 = (((((adc_T >> 4) - ((int32_t)_bmp280m5_calib.dig_T1)) *
              ((adc_T >> 4) - ((int32_t)_bmp280m5_calib.dig_T1))) >>
             12) *
            ((int32_t)_bmp280m5_calib.dig_T3)) >>
           14;
...
float BMP280m5::readPressure() {
    ...
    int32_t adc_P = read24(BMP280m5_REGISTER_PRESSUREDATA);
    adc_P >>= 4;
    ...
    var1 =
        (((((int64_t)1) << 47) + var1)) * ((int64_t)_bmp280m5_calib.dig_P1) >> 33;
 ...
 */
float BMP280m5::readAltitude(float seaLevelhPa) {
...
void BMP280::setSampling(sensor_mode mode, sensor_sampling tempSampling,
    ...
    write8(BMP280m5_REGISTER_CONFIG, _configReg.get());
    write8(BMP280m5_REGISTER_CONTROL, _measReg.get());
}

void BMP280m5::readCoefficients() {
    _bmp280m5_calib.dig_T1 = read16_LE(BMP280m5_REGISTER_DIG_T1);
    _bmp280m5_calib.dig_T2 = readS16_LE(BMP280m5_REGISTER_DIG_T2);
    _bmp280m5_calib.dig_T3 = readS16_LE(BMP280m5_REGISTER_DIG_T3);

    _bmp280m5_calib.dig_P1 = read16_LE(BMP280m5_REGISTER_DIG_P1);
    _bmp280m5_calib.dig_P2 = readS16_LE(BMP280m5_REGISTER_DIG_P2);
    _bmp280m5_calib.dig_P3 = readS16_LE(BMP280m5_REGISTER_DIG_P3);
    _bmp280m5_calib.dig_P4 = readS16_LE(BMP280m5_REGISTER_DIG_P4);
    _bmp280m5_calib.dig_P5 = readS16_LE(BMP280m5_REGISTER_DIG_P5);
    _bmp280m5_calib.dig_P6 = readS16_LE(BMP280m5_REGISTER_DIG_P6);
    _bmp280m5_calib.dig_P7 = readS16_LE(BMP280m5_REGISTER_DIG_P7);
    _bmp280m5_calib.dig_P8 = readS16_LE(BMP280m5_REGISTER_DIG_P8);
    _bmp280m5_calib.dig_P9 = readS16_LE(BMP280m5_REGISTER_DIG_P9);
}
 ...
 */
float BMP280m5::seaLevelForAltitude(float altitude, float atmospheric) {
...
float BMP280m5::waterBoilingPoint(float pressure) {
...
bool BMP280m5::takeForcedMeasurement() {
    ...
    if (_measReg.mode == MODE_FORCED) {
        ...
        while (read8(BMP280m5_REGISTER_STATUS) & 0x08) delay(1);
        return true;
    }
    return false;
}
 ...
 */
void BMP280m5::reset(void) {
    write8(BMP280m5_REGISTER_SOFTRESET, MODE_SOFT_RESET_CODE);
}
 ...
 */
uint8_t BMP280m5::getStatus(void) {
    return read8(BMP280m5_REGISTER_STATUS);
}
